...
beforeEach((done) => {
  const expensesData = {}
  fixtures.forEach(({ id, description, note, amount, createdAt}) => {
...
test('should setup edit expense action object', () => {
  const action = editExpense('123abc', { note: 'edited' })
...
test('should edit expense in firebase', (done) => {
  const store = createMockStore({})
  const id = fixtures[2].id
  const updates = {note : "I am editing this note"}
...
test('should setup add expense action object with provided values', () => {
  const action = addExpense(fixtures[2])
...
test('should setup set expense action object with data', () => {
  const action = setExpenses(fixtures)
  expect(action).toEqual({
    type:'SET_EXPENSES',
    expenses: fixtures
  })
